...
namespace DirRX.Solution.Client
{
  ...
  partial class ContractCollectionActions
  {
    ...
    {
      var lockedContracts = Functions.Contract.Remote.SetStateOnSigning(_objs.ToList<Sungero.Contracts.IContractualDocument>());
      if (lockedContracts.Count == 0)
        ...
        DirRX.Solution.Functions.Module.ShowActionResultDialog(_objs.Count(), lockedContracts, DirRX.Solution.Contracts.Resources.SelectedContractsSetStateOnSigningPartially);
    }
    ...
    {
      Functions.Contract.ExecuteSignedAction(_objs.ToList<Sungero.Contracts.IContractualDocument>());
    }
    ...
    {
      // ? ???????? ????????? ????? ???????????? ????????? ?????????? ??????????.
      var notUpdatedContracts = DirRX.Solution.Functions.Module.Remote.ChangeDocSigningOriginalState(_objs.ToList<Sungero.Contracts.IContractualDocument>(), false);
      if (notUpdatedContracts.Count == 0)
        ...
        DirRX.Solution.Functions.Module.ShowActionResultDialog(_objs.Count(), notUpdatedContracts, DirRX.Solution.Contracts.Resources.SelectedContractsSetStateNotSignedPartially);
    }
    ...
    {
      Functions.Contract.ExecuteCopySignedAction(_objs.ToList<Sungero.Contracts.IContractualDocument>());
    }
  ...
  partial class ContractActions
  {
    public virtual void SendToIMS(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      // Значение типа документа для передачи - "Договор".
      const string contractType = "01";
      ...
      bool isPurchaseFunctionality = _obj.ContractFunctionality == DirRX.Solution.Contract.ContractFunctionality.Purchase;
      bool isSellFunctionality = _obj.ContractFunctionality == DirRX.Solution.Contract.ContractFunctionality.Sale;
      bool isMixedFunctionality = _obj.ContractFunctionality == DirRX.Solution.Contract.ContractFunctionality.Mixed;
      ...
      {
        foreach (ContractsCustom.IIMSContractCode codeIMS in _obj.IMSCodeCollection.Select(c => c.IMSCode))
        {
      ...
      {
        // Закупка.
        foreach (ContractsCustom.IIMSContractCode codeIMS in _obj.IMSCodePurchaseCollection.Select(c => c.IMSCode))
        {
          var kindFunctionality = DirRX.Solution.Structures.Module.SAPContractKindFunctionality.Create();
        ...
        // Сбыт.
        foreach (ContractsCustom.IIMSContractCode codeIMS in _obj.IMSCodeSaleCollection.Select(c => c.IMSCode))
        {
          var kindFunctionality = DirRX.Solution.Structures.Module.SAPContractKindFunctionality.Create();
      ...
      foreach (DirRX.Solution.Structures.Module.SAPContractKindFunctionality kf in kindFunctionalityList.Distinct().ToList())
      {
        ...
        foreach (var ksssContragentId in ksssContragentIdList)        
        {
          ...
          try
          {
            ...
            sapContract.RegNumber = string.Empty;
            sapContract.RegNumberContract = ContractsCustom.PublicFunctions.Module.Remote.GetRegistrationNumberSAP(_obj.RegistrationNumber);
            sapContract.DocumentType = contractType;
     ...
     public virtual void MultipleCounterparties(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      if (_obj.IsManyCounterparties == true && _obj.Counterparties.Count(c => c.Counterparty != null) > 1)
      {
        ...
        dialog.Buttons.AddYesNo();
        if (dialog.Show() == DialogButtons.Yes)
          _obj.IsManyCounterparties = !_obj.IsManyCounterparties;
      }
      else
        _obj.IsManyCounterparties = !_obj.IsManyCounterparties;
    }
    ...
    {
      return _obj.State.Properties.IsManyCounterparties.IsEnabled && _obj.TenderStep != TenderStep.ApprovalProject;
    }
    ...
    public override void SendForApproval(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      bool propertiesEmpty = Functions.Contract.CheckRequiredProperties(_obj, e);
      ...
      else
      {
        ...
        {
          // ?????????????? ?????? ? ?????????? ???? ?????.
          _obj.TotalAmount =  ContractsCustom.PublicFunctions.CurrencyRate.Remote.GetSummInRUB(_obj.TransactionAmount.Value, _obj.Currency);
          _obj.Save();
          base.SendForApproval(e);
        }
        else
          e.AddWarning(DirRX.Solution.Contracts.Resources.SendForApprovalCheckSumMessage);
      }
    }
    ...
    {
      var dialog = Dialogs.CreateInputDialog(_obj.IsStandard == true ? Contracts.Resources.CreateFromTemplate : DirRX.Solution.Contracts.Resources.CreateFromRecommendedForm);
      var title = _obj.IsStandard == true ? DirRX.Solution.Contracts.Resources.StandardForm : DirRX.Solution.Contracts.Resources.RecommendedForm;
      ...
      var standardFormSelect = dialog.AddSelect(title, true, _obj.StandartForm)
        ...
        .Where(s => s.DocumentType.DocumentTypeGuid == DirRX.Solution.PublicConstants.Module.DocumentTypeGuid.Contract)
        ...
        .Where(s => s.Subcategory == null || DirRX.ContractsCustom.ContractSubcategories.Equals(s.Subcategory, _obj.Subcategory))
      ...
      if (template == null)
      {
        e.AddError(DirRX.Solution.Contracts.Resources.NoTemplate);
        return;
      }
    ...
    public virtual bool CanFromTemplate(Sungero.Domain.Client.CanExecuteActionArgs e)
    {
      return _obj.DocumentKind != null &&
        ...
        _obj.Territory != null &&
      	...
      	(_obj.IsTermless == true || (_obj.DocumentValidity != null || _obj.ValidTill != null));
    }

    public virtual void OpenProductName(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      DirRX.Brands.PublicFunctions.ProductName.Remote.GetProductNames().Show();
    }

    public virtual bool CanOpenProductName(Sungero.Domain.Client.CanExecuteActionArgs e)
    {
      return true;
    }
